...
def build_cell(cell, compiler, mode) -> JupyterCell:
    ...
    return JupyterCell(
        name=cell_name, code=to_run, compiler=compiler, metadata=metadata
    )
...
def get_from_file(filename):
    with open(os.path.join(Path(__file__).parent, "testdata", filename)) as f:
        return f.readlines()


def get_output(outputs, name) -> Any:
    return (
        [v for v in outputs[name]]
        if isinstance(outputs[name], MutableSequence)
        else outputs[name]
    )
...
async def run(notebook, translator, user_ns) -> MutableMapping[str, Any]:
    ...
    output_tasks = {
        posixpath.split(name)[1]: asyncio.create_task(port.get(random_name()))
        for name, port in workflow.get_output_ports().items()
    }
    return dict(
        zip(
            output_tasks.keys(),
            [get_token_value(t) for t in await asyncio.gather(*output_tasks.values())],
        )
    )


def set_code(workflow_cell: MutableMapping[str, Any], code: MutableSequence[str]):
    workflow_cell["source"] = code


def set_inputs(
    workflow_cell: MutableMapping[str, Any],
...
def set_scatter(
    workflow_cell: MutableMapping[str, Any], scatter: MutableMapping[str, Any]
):
...
def translator(context):
    return JupyterNotebookTranslator(
        context=context, output_directory=tempfile.mkdtemp()
    )
...
def workflow_cell():
    return {
        "cell_type": "code",
        "metadata": {"workflow": {"step": {"in": [], "out": []}, "version": "v1.0"}},
        "source": [],
    }


class MockDisplayHook:
