...
import { IWorkflowRequestMsg } from "./messages";
...
namespace Private {
  ...
  export function execute(
    ...
    const content: IWorkflowRequestMsg['content']['notebook'] = {
    ...
    console.log(`Sending 'workflow_request' message ${JSON.stringify(content)} to the kernel`)
  }
  ...
  export function runWorkflow(
    ...
    const promises: Promise<boolean>[] = cells.map(cell => {
      if (
    ...
    if (workflowCells.length > 0) {
      execute(workflowCells, notebook.model?.getMetadata('workflow'), sessionContext);
    }
