...
import {
  ...
  type INotebookModel,
  KernelError, StaticNotebook
} from "@jupyterlab/notebook";
import type { KernelMessage } from '@jupyterlab/services';
import { nullTranslator } from '@jupyterlab/translation';
import { findIndex } from '@lumino/algorithm';
import type { JSONObject } from '@lumino/coreutils'
import INotebookConfig = StaticNotebook.INotebookConfig;

export class WorkflowCellExecutor implements INotebookCellExecutor {
  async runCell({
    cell,
    notebook,
    notebookConfig,
        ...
        if (sessionContext) {
          ...
          try {
            const reply = await CodeCell.execute(
              cell as CodeCell,
              sessionContext,
              handleMetadata(notebook, cell, deletedCells)
            );
