  ...
  return {
    onload: function () {
      ...
      CodeCell.prototype.execute = function (stop_on_error) {
        if (!this.kernel) {
          console.log(
            i18n.msg._("Can't execute cell since kernel is not set.")
          );
          return;
        }
        if (this.element.hasClass('running')) {
          console.log(
            i18n.msg.sprintf('Cell %s is already running.', this.cell_id)
          );
          return;
        }
        if (stop_on_error === undefined) {
          if (this.metadata !== undefined && this.metadata.tags !== undefined) {
            stop_on_error =
              this.metadata.tags.indexOf('raises-exception') === -1;
          } else {
            stop_on_error = true;
          }
        }
        ...
        if (
          ...
          if (
            this.notebook.metadata !== undefined &&
            this.notebook.metadata.hasOwnProperty('workflow')
          ) {
            ...
            kernel_options.workflow = JSON.parse(
              JSON.stringify(this.metadata['workflow'])
            );
          }
        ...
        if (background) {
          ...
          this.kernel.events.trigger('background_execution_request.Kernel', {
            kernel: this,
            content: content
          });
          return this.kernel.send_shell_message(
        ...
        function handleFinished(evt, data) {
          if (
            that.kernel.id === data.kernel.id &&
            that.last_msg_id === data.msg_id
          ) {
      ...
      CodeCell.prototype.get_auto_inputs = function (container) {
        if (!this.kernel) {
          console.log(
            i18n.msg._("Can't retrieve cell inputs since kernel is not set.")
          );
          return;
        }
      ...
      CodeCell.prototype._handle_auto_inputs = function (container, msg) {
        if (msg.content.inputs.length === 0) {
          container.append(
            $('<span/>').text('No input dependencies retrieved.')
          );
        } else {
          let names = [];
          container
          ...
          msg.content.inputs.forEach(input => {
            container.append(
              $('<span/>')
                .addClass('label')
                .addClass(
                  names.includes(input) ? 'label-success' : 'label-info'
                )
                .css('margin', '2px')
                .css('font-size', '12px')
      ...
      Notebook.prototype.execute_workflow = function () {
        ...
        const that = this;
        const cells = [...Array(this.ncells()).keys()]
          .map(function (index) {
        ...
        cells.forEach(function (cell) {
          ...
          if (
            ...
            cell.metadata.hasOwnProperty('workflow')
          ) {
            cell_object.metadata = JSON.parse(
              JSON.stringify(cell.metadata['workflow'])
            );
          } else {
        ...
        this.kernel.events.trigger('workflow_request.Kernel', {
          kernel: this,
          content: content
        });
        return this.kernel.send_shell_message(
      ...
      const edit_workflow_action = {
        handler: function (env) {
          edit_workflow({
            md:
              'workflow' in env.notebook.metadata
                ? env.notebook.metadata['workflow']
                : {},
      ...
      Jupyter.toolbar.add_buttons_group([
        {
          action: edit_workflow_action_name,
          label: i18n.msg._('Edit Workflow')
        },
        ...
        $('<div/>')
          ...
          .append(
            $('<label/>')
              ...
              .append($('<span/>').text(i18n.msg._('From name')))
          )
          .append(
            $('<a/>')
              .attr('href', '#')
            ...
            $('<div/>')
              ...
              .append(
                $('<label/>')
                  ...
                  .append($('<span/>').text(i18n.msg._('External')))
              )
          )
          .appendTo(deployment_el);
        let config_div = $('<div/>')
      ...
      const edit_workflow = function (options) {
        const deployments =
          'deployments' in options.md ? options.md['deployments'] : {};
        const serializers =
          'serializers' in options.md ? options.md['serializers'] : {};
        ...
        for (let name in deployments) {
          ...
          const external =
            'external' in deployment ? deployment['external'] : false;
          const config =
            'config' in deployment
              ? JSON.stringify(deployment['config'], null, 2)
              : '';
          const deployment_html = get_deployment_html(
            name,
            type,
            external,
            config
        ...
        for (let name in serializers) {
          ...
          const postload =
            'postload' in serializer ? serializer['postload'] : false;
        ...
        const dialogform = $('<div/>').append(
          $('<form/>')
            .addClass()
            .append($('<fieldset/>').append(error_div).append($('<br/>')))
            .append(
              $('<fieldset/>')
        ...
        $('body')
          .on('click', '.btn-add-deployment', function (event) {
            ...
            if (name === '') {
              error_div.text(
                i18n.msg._(
                  'WARNING: Specify the deployment name you want to add.'
                )
              );
              error_div.focus();
            } else {
              ...
              if (defined_deployments.includes(name)) {
                error_div.text(
                  i18n.msg._('WARNING: This deployment is already defined.')
                );
                error_div.focus();
              } else {
                error_div.text('');
          ...
          .on('click', '.btn-add-serializer', function (event) {
            ...
            if (name === '') {
              error_div.text(
                i18n.msg._(
                  'WARNING: Specify the serializer name you want to add.'
                )
              );
              error_div.focus();
            } else {
              ...
              if (defined_serializers.includes(name)) {
                error_div.text(
                  i18n.msg._('WARNING: This serializer is already defined.')
                );
                error_div.focus();
              } else {
                error_div.text('');
          ...
          .on('click', '.btn-delete', function (event) {
            ...
            dialog.modal({
              ...
              body: i18n.msg.sprintf(
                "Are you sure you want to delete the '%s' element?",
                name
              ),
            ...
            Reset: {
              ...
              click: function () {
                dialog.modal({
                  ...
                  body: i18n.msg._(
                    'Are you sure you want to restore the default configuration for this cell?'
                  ),
                  default_button: 'Cancel',
                  buttons: {
                    Cancel: {},
                    ...
                    try {
                      new_md['deployments'][name] = {
                        type: $(this)
                          ...
                          .val(),
                        external: $(this)
                          .find('.deployment-external')
                          .prop('checked')
                      };
                      const config_string = config_editors[name].getValue();
                      if (config_string !== '') {
                        new_md['deployments'][name].config =
                          JSON.parse(config_string);
                      }
                    } catch (e) {
                      console.log(e);
                      error_div.text(
                        i18n.msg._('WARNING: Invalid deployment config JSON.')
                      );
                      event.preventDefault();
                      event.stopImmediatePropagation();
                    }
        ...
        modal_obj.find('.modal').css('display', 'block');
        modal_obj.find('.modal-dialog').css('overflow-y', 'initial');
        modal_obj
          .find('.modal-body')
          .css('height', '80vh')
          .css('overflow-y', 'auto');
      ...
      const edit_step = function (options) {
        const interpreter =
          'interpreter' in options.md ? options.md['interpreter'] : '';
        const serializers =
          'serializers' in options.notebook_md
            ? options.notebook_md['serializers']
            : [];
        ...
        const scatter =
          'scatter' in step ? JSON.stringify(step['scatter'], null, 2) : '';
        ...
        inputs.forEach(input => {
          ...
          const serializer =
            'serializer' in input ? input['serializer'] : undefined;
        ...
        outputs.forEach(output => {
          ...
          const serializer =
            'serializer' in output ? output['serializer'] : undefined;
          ...
          const output_html = get_dep_html(
            'out',
            name,
            serializers,
            serializer,
        ...
        const deployments =
          'deployments' in options.notebook_md
            ? options.notebook_md['deployments']
            : {};
        ...
        const dialogform = $('<div/>').append(
          $('<form/>')
                  ...
                  $('<div/>')
                    ...
                    .append(
                      $('<label/>')
                        ...
                        .append(
                          $('<span/>').text(i18n.msg._('Execute in background'))
                        )
                        .append(
                          $('<a/>')
                            .attr('href', '#')
                      ...
                      $('<div/>')
                        ...
                        .append(
                          $('<button/>')
                            ...
                            .append($('<i/>').addClass('fa-eye').addClass('fa'))
                        )
                    )
                )
                .append(inputs_html)
            ...
            .append(
              $('<fieldset/>')
                .append($('<legend/>').text(i18n.msg._('Target')))
                .append(
                  $('<div/>')
                    .addClass('form-group')
                    .append(
        ...
        $('body')
          .on('click', '.btn-add-input', function (event) {
            ...
            if (name === '') {
              error_div.text(
                i18n.msg._('WARNING: Specify the input name you want to add.')
              );
              error_div.focus();
            } else {
              let defined_names = [];
              ...
              if (defined_names.includes(name)) {
                error_div.text(
                  i18n.msg._('WARNING: This name is already defined.')
                );
                error_div.focus();
              } else {
                error_div.text('');
                $('.in-container').append(
                  get_dep_html('in', name, serializers)
                );
                $(event.currentTarget).parent().prev().val('');
          ...
          .on('click', '.btn-add-output', function (event) {
            ...
            if (name === '') {
              error_div.text(
                i18n.msg._('WARNING: Specify the output name you want to add.')
              );
              error_div.focus();
              return false;
            } else {
              ...
              if (defined_names.includes(name)) {
                ...
                error_div.text('');
                $('.out-container').append(
                  get_dep_html('out', name, serializers)
                );
                $(event.currentTarget).parent().prev().val('');
              }
              $('.step-background')
          ...
          .on(
            'click',
            '.btn-view-inputs',
            { notebook: options.cell.notebook },
            function (event) {
              event.stopImmediatePropagation();
              event.preventDefault();
              ...
              if (container.length) {
                button
                  ...
                  .prop('disabled', false);
                button
                  .removeClass('btn-view-inputs')
                  .addClass('btn-hide-inputs');
                button
                  .find('.fa')
                  .removeClass('fa-eye')
            ...
            Edit: {
              ...
              click: function (event) {
                ...
                if (!background.prop('disabled')) {
                  new_md['step']['background'] = dialogform
                    .find('.step-background')
                    .prop('checked');
                } else {
                ...
                try {
                  ...
                  console.log(e);
                  error_div.text(
                    i18n.msg._('WARNING: Invalid scatter config JSON.')
                  );
                  event.preventDefault();
                  event.stopImmediatePropagation();
                }
                new_md['step']['autoin'] = dialogform
                  .find('.step-autoin')
                  .prop('checked');
                new_md['step']['out'] = [];
                dialogform
                  ...
                  .each(function () {
                    ...
                    try {
                      ...
                      console.log(e);
                      error_div.text(
                        i18n.msg._('WARNING: Invalid output scatter JSON.')
                      );
                      event.preventDefault();
                      event.stopImmediatePropagation();
                    }
                ...
                if (deployment !== '') {
                  new_md['target'] = {
                    ...
                    locations: parseInt(
                      dialogform.find('.target-locations').val()
                    ),
      ...
      let step_edit = function (cell, edit_step_button) {
        edit_step({
          ...
          notebook_md:
            'workflow' in cell.notebook.metadata
              ? cell.notebook.metadata['workflow']
              : {},
          callback: function (md) {
            if ($.isEmptyObject(md)) {
      ...
      const add_step_edit_button = function (div, cell) {
        ...
        const button = $('<button/>')
          ...
          .append($('<i/>').addClass('fa-gear').addClass('fa'))
          .append(
            $('<span/>')
      ...
      CellToolbar.register_callback('default.stepedit', add_step_edit_button, [
        null,
        'code'
      ]);
      CellToolbar.register_preset(
        i18n.msg._('Edit Workflow Step'),
        example_preset,
        notebook
      );
